...
export class TicketUpdatePage {
  ...
  ticketInput = element(by.id('field_ticket'));
  ...
  async setTicketInput(ticket: string): Promise<void> {
    await this.ticketInput.sendKeys(ticket);
  }

  async getTicketInput(): Promise<string> {
    return await this.ticketInput.getAttribute('value');
  }
